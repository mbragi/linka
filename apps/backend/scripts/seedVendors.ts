...
async function seedVendors() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.DATABASE_URL || 'mongodb://localhost:27017/linka');
    ...
    for (const vendorData of sampleVendors) {
      try {
        // Create wallet
        const { address: walletAddress, encryptedPrivateKey } = await walletService.createWallet();
